...
preparadas sem decisão são descartadas com segurança. Snapshots periódicos
compactam o log.
...
venceu a eleição e assumiu em $\approx$4\,s (janela dominada pelo
\emph{timeout} de detecção, de 3\,s); a réplica restante ressincronizou as
594 chaves do novo líder em seguida. Também observamos a reexecução
...
a janela de reeleição ($\approx$4\,s), os clientes reexecutam as escritas
...
pela implementação do nó (servidor gRPC) e pela integração dos módulos,
incluindo a coordenação das escritas via 2PC.
...
Juan Pablo Ramos de Oliveira foi responsável pela documentação do projeto
(instruções de execução e relatório) e pelo roteiro de demonstração.
